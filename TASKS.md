...
## TASK-012: Поиск по тексту торговых объявлений ✅ ВЫПОЛНЕНО 22.02.2026

**Реализовано:**

- `SEARCH_QUERIES` — 6 запросов с "Москва" + тип объекта
- `search_via_trade_messages` — итерирует по запросам, дедупликация по GUID
- `trade_messages?search={query}&publishedAfter={date}` — основной эндпоинт
- `search_lots` — использует только text search, ≤50 запросов/цикл
- Логи показывают результат каждого запроса

**Ждёт проверки:** следующий запуск в 03:00 МСК (сброс лимита)
